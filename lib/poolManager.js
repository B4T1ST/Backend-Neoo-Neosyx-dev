...
module.exports = {
 ...
 get: (name, config) => {
  if (!pools.has(name)) {
   ...
   // Adicione a propriedade connectionTimeout ao objeto de configuração
   config.connectionTimeout = 60000; // 60 segundos, por exemplo

   const pool = new mssql.ConnectionPool(config);

   // automaticamente remove o pool do cache se `pool.close()` for chamado
