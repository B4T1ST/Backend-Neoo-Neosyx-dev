...
//Rota que retorna os links da página principal
router.get('/', function(req, res) {
    ...
    retornaLinks (loginUsuario,pesquisa,res)
});

async function retornaLinks(loginUsuario,pesquisa,res){
    try {
        // Requisição do banco
        let result = await global.conn.request()
            ...
            //executa a procedure  
            .execute('s_Retorna_Itens_Monitoria')
        
        if (!result.recordsets) {
            res.status(500).json('Não foi possível retornar os links.')
        }

        //Puxando array com as informações das coleções   
        let links = result.recordset 
