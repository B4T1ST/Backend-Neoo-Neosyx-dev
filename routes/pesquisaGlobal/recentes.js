...
router.get('/', function(req, res) {
  ...
  if (!pesquisa) {
    res.status(400).json('pesquisas recentes não encontradas.')
    return
  }


  retornaRecentes(loginUsuario, pesquisa, res)
});

async function retornaRecentes(loginUsuario, pesquisa, res){
  try {
    // Requisição do banco
    let result = await global.conn.request()
        ...
        //executa a procedure  
        .execute('s_Pesquisa_Global_Retorna_Recentes')
